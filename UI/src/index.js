...
if (food || foodCart) {
  window.addEventListener('scroll', () => {
    if (window.scrollY >= 370 && window.innerWidth > 991) {
      ...
      food.classList.add('foodNavWrapper-onWindowScroll');
      foodItemContainer.classList.add('-offset-m-3x');
    } else if (window.scrollY < 360 && window.innerWidth < 991) {
      food.classList.remove('foodNavWrapper-onWindowScroll');
      foodItemContainer.classList.remove('-offset-m-3x');
    }
...
if (closeF)   {if (headerPane) {
  window.addEventListener('scroll', () => {
    if (window.scrollY > 40) {
      ...
      headerPane.classList.add('header-with-fixed-position');
    } else if (window.scrollY < 40) {
      headerPane.classList.remove('header-with-fixed-position');
